-- CRITICAL SECURITY FIX: Enhanced data masking for customer personal information
-- Create comprehensive masking functions for order data

-- Enhanced name masking function
...
-- Create secure order access function with comprehensive masking
...
-- Update the existing get_orden_with_masking function to use enhanced name masking
CREATE OR REPLACE FUNCTION public.get_orden_with_masking(orden_id uuid)
RETURNS TABLE(
  id uuid, numero_orden text, estado text, 
  remitente_nombre text, remitente_documento_masked text, remitente_domicilio_masked text, 
  remitente_localidad text, remitente_provincia text, 
  destinatario_nombre text, destinatario_documento_masked text, destinatario_domicilio_masked text, 
  destinatario_localidad text, destinatario_provincia text, 
  access_level text, created_at timestamp with time zone
)
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO ''
...
BEGIN
  ...
  -- Return masked data using enhanced name masking
  RETURN QUERY SELECT
    orden_record.id,
    orden_record.numero_orden,
    orden_record.estado,
    public.mask_nombre(orden_record.remitente_nombre) as remitente_nombre,
    ...
    public.mask_nombre(orden_record.destinatario_nombre) as destinatario_nombre,
